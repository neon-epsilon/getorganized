...
if(!empty($_POST['hoursofwork_delete_submit'])){
    ...
    // create info message
    $info_message = "<p class=\"smallinfo\">Einträge <br>" . $ids_string . "<br>in hoursofwork gelöscht.</p>";
...
// Get the last entry
if (! $result = $mysqli->query("SELECT id, date, amount, category FROM hoursofwork ORDER BY id DESC LIMIT {$number_of_items}" ) ) {
...
echo "</table>";
echo '<input class="form_button" type="submit" name="hoursofwork_delete_submit" value="Löschen"/>';
