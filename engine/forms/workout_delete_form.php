...
if(!empty($_POST['workout_delete_submit'])){
    ...
    // create info message
    $info_message = "<p class=\"smallinfo\">Einträge <br>" . $ids_string . "<br>in workout gelöscht.</p>";

    /* rebuild workout output */
    exec($_SERVER["DOCUMENT_ROOT"] . '/engine/reporting/build_workout_output.py > /dev/null 2> /dev/null &');
}

// Get the last entry
if (! $result = $mysqli->query("SELECT id, date, amount, category FROM workout ORDER BY id DESC LIMIT {$number_of_items}" ) ) {
...
echo "</table>";
echo '<input class="form_button" type="submit" name="workout_delete_submit" value="Löschen"/>';
